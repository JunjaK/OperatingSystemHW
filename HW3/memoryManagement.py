...
def request(mem, id, size, holeTable, allocatedTable, memSize):
  ...
  allocatedTable.sort(key=allocatedTableSorting)
...
def main():
  ...
  allocatedTable = []
  while True:
    operation = int(input('\n실행할 연산을 입력해주세요. (0번 - 프로그램 종료, 1번 - Request) : '))
    if(operation == 0):
      print('프로그램을 종료합니다. \n')
      break
    elif(operation == 1):
      ...
      while True:
        checkDup = False
        requsetID, requsetSize  = map(int, (input('Request ID와 Request Size를 입력해주세요(Size가 0일 경우 free request) : ').split()))
        if(requsetSize != 0):
          for i in range(0, len(allocatedTable)):
      ...
      if(requsetSize != 0):
        ...
        # free request 실행
        memory, holeTable, allocatedTable, remainMemory = freeReuest(memory, requsetID, holeTable, allocatedTable, memorySize)
